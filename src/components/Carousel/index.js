...
export default function Carousel({ children, refContainer }) {
  ...
  const onDragMove = useCallback(
    (e) => {
      ...
      refDragHandler.current.style.left = `${
        refDragHandler.current.offsetLeft - postX2.current
      }px`;
    },
    [postX1, postX2]
  );
  ...
  useLayoutEffect(() => {
    const refForwardDragHanlder = refDragHandler.current;

    refForwardDragHanlder.onmousedown = onDragStart;
    refForwardDragHanlder.addEventListener("touchstart", onDragStart);
    refForwardDragHanlder.addEventListener("touchend", onDragEnd);
    refForwardDragHanlder.addEventListener("touchmove", onDragMove);
    refForwardDragHanlder.addEventListener("click", onClick);
    refForwardDragHanlder.addEventListener("transitionend", fnCheckIndex);
    return () => {
      refForwardDragHanlder.removeEventListener("touchstart", onDragStart);
      refForwardDragHanlder.removeEventListener("touchend", onDragEnd);
      refForwardDragHanlder.removeEventListener("touchmove", onDragMove);
      refForwardDragHanlder.removeEventListener("click", onClick);
      refForwardDragHanlder.removeEventListener("transitionend", fnCheckIndex);
    };
