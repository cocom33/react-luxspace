      ...
      >
        <div className="bg-gray-300 rounded-lg w-full h-full">
          <div className={`overlay ${ratio?.meta?.[item.ratio.md]}`}>
            <div className="w-24 h-3 rounded-full bg-gray-400 mt-3"></div>
            <div className="w-36 h-3 rounded-full bg-gray-400 mt-2"></div>
          </div>
        </div>
      </div>
    );
...
export default function BrowseRoom() {
  ...
  const ratioClassName = {
    ...
    meta: {
      "1/9":
        "md:-left-10 top-0 bottom-0 flex justify-center flex-col pl-48 md:pl-72",
      "1/4":
        "md:-left-10 top-0 bottom-0 flex justify-center flex-col pl-48 md:pl-72",
      "2/2":
