...
export default function JusrArrive() {
  const { data, error, run, isLoading } = useAsync();

  const refContainer = useRef(null);

  useEffect(() => {
    run(fetchData({ url: "/api/products/?page=1&limit=12" }));
  }, [run]);
            ...
            {data.data.map((item) => {
              return (
                <div className="px-4 relative card group" key={item.id}>
                  ...
                  >
                    <div className="absolute opacity-0 group-hover:opacity-100 transition duration-200 flex items-center justify-center w-full h-full bg-black/50 opacity-35">
